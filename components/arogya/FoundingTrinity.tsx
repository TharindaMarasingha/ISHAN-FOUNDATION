        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {YOGINIS.map((yogini, index) => (
            <RevealOnScroll key={yogini.name} delay={index * 0.15} className="h-full">
              <div className="h-full flex flex-col items-center text-center p-8 sm:p-10 border border-burntOrange/20 bg-darkBrown/5 transition-all duration-300 hover:-translate-y-1.5 hover:border-burntOrange/50 hover:shadow-[0_10px_30px_rgba(46,26,14,0.05)]">
                
                <span className="font-display text-4xl text-sacredGold mb-6 leading-none">
                  {yogini.symbol}
                </span>

                <h4 className="font-display text-2xl text-deepAmber mb-2">
                  {yogini.name}
                </h4>

                <p className="font-display italic text-lg text-burntOrange mb-6">
                  {yogini.alias}
                </p>

                <div className="mb-6 px-4 py-1.5 rounded-full border border-sacredGold/40 bg-sacredGold/5 text-[0.65rem] uppercase tracking-widest text-sacredGold">
                  {yogini.energy}
                </div>
